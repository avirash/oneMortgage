...
class Chrome {
  ...
  async navigate(url) {
    ...
    await this.page.goto(url, { timeout: 20000 }).then((gotoResult) => { })
  ...
  async run(input) {
    console.log('description: ' + typeof input.description  )
    console.log(input.description instanceof Array);
    ...
    for (const action of description) {
      if (action.waitFor) {
        this.waitfor = action.waitFor
          ...
          console.log('when: ' + await this.$(`${action.when}`))
