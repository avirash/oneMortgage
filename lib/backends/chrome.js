...
class Chrome {
  ...
  async handleInputTag(selector, val) {
    try {
      let self = this
      await this.page.evaluate(async(css, val, waitfor) => {
  ...
  try {
      ...
      for (const action of description) {
        if (action.delay) {
          await this.waitForOld(action.delay)
        ...
        await this.scroll()
          if (action.action === 'click') {
            ...
            await this.waitForOld(this.waitfor)
            // await new Promise((resolve) => {
  ...
  async waitForOld(conditions, context = null) {
    await Promise.delay(conditions)
  }
